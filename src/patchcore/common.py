...
class RescaleSegmentor:
    ...
    def convert_to_segmentation(self, patch_scores):
        ...
        #
        # return [np.where(patch_score > 7, patch_score, 0) for patch_score in patch_scores]

        ##### 大缺陷加入全局阈值的地方 ####
            patch_scores = [ndimage.gaussian_filter(patch_score, sigma=self.smoothing)for patch_score in patch_scores]

        return [np.where(patch_score > 0.3, patch_score, 0) for patch_score in patch_scores]
